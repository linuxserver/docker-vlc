pipeline {
  ...
  environment {
    ...
    BUILD_VERSION_ARG = 'VLC_VERSION'
    LS_USER = 'linuxserver'
    LS_REPO = 'docker-vlc'
    CONTAINER_NAME = 'vlc'
    DOCKERHUB_IMAGE = 'linuxserver/vlc'
    DEV_DOCKERHUB_IMAGE = 'lsiodev/vlc'
    PR_DOCKERHUB_IMAGE = 'lspipepr/vlc'
    DIST_IMAGE = 'arch'
    MULTIARCH = 'true'
    CI = 'true'
    CI_WEB = 'true'
  ...
  stages {
    ...
    stage("Set tag custom bash"){
      steps{
        script{
          env.EXT_RELEASE = sh(
            script: ''' curl -sL https://mirror.rackspace.com/archlinux/extra/os/x86_64/extra.db | tar tzf - | awk -F '(-|/)' '/vlc/ {print $2; exit}' ''',
            returnStdout: true).trim()
    ...
    stage('Build-Single') {
      ...
      steps {
        ...
        sh "docker buildx build \
          ...
          --label \"org.opencontainers.image.url=https://github.com/linuxserver/docker-vlc/packages\" \
          --label \"org.opencontainers.image.documentation=https://docs.linuxserver.io/images/docker-vlc\" \
          --label \"org.opencontainers.image.source=https://github.com/linuxserver/docker-vlc\" \
          --label \"org.opencontainers.image.version=${EXT_RELEASE_CLEAN}-ls${LS_TAG_NUMBER}\" \
          --label \"org.opencontainers.image.revision=${COMMIT_SHA}\" \
          --label \"org.opencontainers.image.vendor=linuxserver.io\" \
          --label \"org.opencontainers.image.licenses=GPL-3.0-only\" \
          --label \"org.opencontainers.image.ref.name=${COMMIT_SHA}\" \
          --label \"org.opencontainers.image.title=Vlc\" \
          --label \"org.opencontainers.image.description=[VLC Media Player](https://www.videolan.org/vlc/) is a free and open source cross-platform multimedia player and framework that delivers dependable performance across multiple devices.\" \
